...
public abstract class ProvToDotEdited {
    ...
    protected ProvUtilities u=new ProvUtilities();
    ProvFactory pf=new ProvFactory();
    
    QualifiedName SUM_SIZE=pf.newQualifiedName("http://openprovenance.org/provtoolbox/" + "summary/ns#", "size", "sum"); 
    ...
    public ProvToDotEdited(Config config) {
        InputStream is=null;
        switch (config) {
        case DEFAULT:
    ...
    public ProvToDotEdited(String configurationFile) {
        this();
        init(configurationFile);
    }

    public ProvToDotEdited(String configurationFile, String other) {
    ...
    public void emitAnnotations(String id, HasOther statement, PrintStream out) {
        ...
        HashMap<String,String> properties=new HashMap<String, String>();
        addStepComment(properties, statement);
        ...
        addStepComment(linkProperties, statement);
    ...
    public abstract void addStepComment(HashMap<String,String> properties, HasOther statement);
	...
	Hashtable<String,List<Other>> table=u.attributesWithNamespace(object,"http://openprovenance.org/provtoolbox/"+"dot/ns#");
	...
	o=table.get("size");
	if (o!=null && !o.isEmpty()) {
	    if ((object instanceof QualifiedRelationEdited)) {
    ...
    public HashMap<String,String> addAnnotationLabel(HasOther ann, HashMap<String,String> properties) {
		...
		if (prop.getElementName().getNamespaceURI().startsWith("http://openprovenance.org/provtoolbox/")) {
	...
	for (Other obj: ann.getOther()) {
	    if (!(obj.getElementName().getNamespaceURI().startsWith("http://openprovenance.org/provtoolbox/"))) {
		count++;
	    }
	}
	return count;
   ...
   public String nonEmptyLocalName(QualifiedName name) {
    ...
    public void emitDependency(Relation e, PrintStream out) {
		...
		if (e instanceof QualifiedRelationEdited) {
		    addColors((QualifiedRelationEdited)e,properties);
		}
    ...
    protected void relationName(Relation e, HashMap<String,String> properties) {
    ...
    public void addRelationLabel(Relation e0, HashMap<String,String> properties) {
        String label=null;
        if (!(e0 instanceof QualifiedRelationEdited)) return;
        QualifiedRelationEdited e=(QualifiedRelationEdited)e0;
